...
void Interface_DrawItemButtons(GlobalContext* globalCtx) {
    ...
    s16 L_BTN_Pos[]    = { L_BUTTON_X+Right_HUD_Margin, L_BUTTON_Y+(Top_HUD_Margin*-1) };
    ...
    if (CVar_GetS32("gHudColors", 1) == 0) {
        gDPSetPrimColor(OVERLAY_DISP++, 0, 0, R_C_BTN_COLOR(0), R_C_BTN_COLOR(1), R_C_BTN_COLOR(2), interfaceCtx->cUpAlpha);
    } else if (CVar_GetS32("gHudColors", 1) == 1) {
        gDPSetPrimColor(OVERLAY_DISP++, 0, 0, R_C_BTN_COLOR(0), R_C_BTN_COLOR(1), R_C_BTN_COLOR(2), interfaceCtx->cUpAlpha);
    } else if (CVar_GetS32("gHudColors", 1) == 2) {
        gDPSetPrimColor(OVERLAY_DISP++, 0, 0, CVar_GetS32("gCCCBtnPrimR", R_C_BTN_COLOR(0)), CVar_GetS32("gCCCBtnPrimG", R_C_BTN_COLOR(1)), CVar_GetS32("gCCCBtnPrimB", R_C_BTN_COLOR(2)), interfaceCtx->cUpAlpha);
    }
    gSPWideTextureRectangle(OVERLAY_DISP++, OTRGetRectDimensionFromRightEdge(C_Up_BTN_Pos[0]) << 2, C_Up_BTN_Pos[1] << 2,
                        (OTRGetRectDimensionFromRightEdge(C_Up_BTN_Pos[0]) + R_ITEM_BTN_WIDTH(2)) << 2,
                        (C_Up_BTN_Pos[1] + R_ITEM_BTN_WIDTH(2)) << 2,
                        G_TX_RENDERTILE, 0, 0, R_ITEM_BTN_DD(2) << 1, R_ITEM_BTN_DD(2) << 1);
    ...
    if (interfaceCtx->naviCalling && (globalCtx->pauseCtx.state == 0) && (globalCtx->pauseCtx.debugState == 0) &&
        ...
        if (!sCUpInvisible) {
            ...
            const s16 rLBtnX  = OTRGetRectDimensionFromRightEdge(R_L_BTN_X+Right_HUD_Margin);
            const s16 rLIconX = OTRGetRectDimensionFromRightEdge(R_L_ICON_X+Right_HUD_Margin-!!CVar_GetS32("gNaviTextFix", 0));
            if (CVar_GetS32("gHudColors", 1) == 0) {
            ...
            gDPSetCombineMode(OVERLAY_DISP++, G_CC_MODULATEIA_PRIM, G_CC_MODULATEIA_PRIM);
            gSPWideTextureRectangle(OVERLAY_DISP++, rLBtnX << 2, R_L_BTN_Y + (Top_HUD_Margin * -1) << 2,
                                    (rLBtnX + 16) << 2, (R_L_BTN_Y+(Top_HUD_Margin*-1) + 16) << 2, G_TX_RENDERTILE, 0, 0, 2 << 10, 2 << 10);
            gDPPipeSync(OVERLAY_DISP++);
            ...
            gSPWideTextureRectangle(OVERLAY_DISP++, rLIconX << 2, R_L_ICON_Y+(Top_HUD_Margin*-1) << 2, (rLIconX + 32) << 2,
                                (R_L_ICON_Y+(Top_HUD_Margin*-1) + 8) << 2, G_TX_RENDERTILE, 0, 0, 1 << 10, 1 << 10);
        }
        ...
        if (gSaveContext.equips.buttonItems[temp] > 0xF0) {
            ...
            if (temp==4) {
                OVERLAY_DISP = Gfx_TextureIA8(OVERLAY_DISP, ((u8*)gButtonBackgroundTex), 32, 32, 
                                              OTRGetRectDimensionFromRightEdge(C_UP_BUTTON_X+Right_HUD_Margin), C_UP_BUTTON_Y+(Top_HUD_Margin*-1), R_ITEM_BTN_WIDTH(2),
...
void Interface_DrawItemIconTexture(GlobalContext* globalCtx, void* texture, s16 button) {
    ...
    if (button == 4) {
        gSPWideTextureRectangle(OVERLAY_DISP++, OTRGetRectDimensionFromRightEdge(C_UP_BUTTON_X+Right_HUD_Margin) << 2, C_UP_BUTTON_Y+(Top_HUD_Margin*-1) << 2,
                            (OTRGetRectDimensionFromRightEdge(C_UP_BUTTON_X+Right_HUD_Margin) + R_ITEM_ICON_WIDTH(2)) << 2,
                            (C_UP_BUTTON_Y+(Top_HUD_Margin*-1) + R_ITEM_ICON_WIDTH(2)) << 2, G_TX_RENDERTILE, 0, 0,
